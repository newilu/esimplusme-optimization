...
function PhoneNumbersByRegion({
  ...
  React.useEffect(() => {
    if (typeof phone === 'string' && phoneNumber !== null && phoneNumber.phoneNumber !== phone) {
      replace({ query: { ...rest, phone: phoneNumber?.phoneNumber } }, undefined, { shallow: true, scroll: false });
    }
  }, []);
      ...
      <SectionsWrapper>
        {phoneNumber ? (
          <PanelSection>
            ...
            <PhoneNumberPurchase state={state} phone={phoneNumber} country={country} />
          </PanelSection>
        ) : (
          <>
            <PanelSection>
              <PanelSectionTitle>
                ...
                <Link href={`/virtual-phone-number/${formatStringToKebabCase(country.name)}`}>{t('change')}</Link>
              </PanelSectionTitle>
              <PanelSectionTitle style={{ padding: '15px 25px' }}>
                {cities.length ? t('cities') : t('all_numbers')}
              </PanelSectionTitle>
              {!!cities.length && <CitiesTable areaCode={areaCode} cities={cities} />}
              {!cities.length &&
                (phones.length ? (
                  <PhoneNumbersTable
                    onRowClick={(data) => {
