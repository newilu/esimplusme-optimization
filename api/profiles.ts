...
function listCountries() {
  return queryFetcher<{ data: { countries: Country[] } }>(MAIN_API_URL.concat(ENDPOINTS.countries()));
}

function getCountryByIsoName(isoName: string) {
  return queryFetcher<{ data: { country: CountryByISO } }>(MAIN_API_URL.concat(ENDPOINTS.country(isoName)));
}

function listRegions() {
  return queryFetcher<{ data: { regions: Region[] } }>(MAIN_API_URL.concat(ENDPOINTS.regions()));
}

function getWorldwideRegion() {
  return queryFetcher<{ data: { region: RegionById } }>(MAIN_API_URL.concat(ENDPOINTS.worldwideRegion()));
}

function getRegionById(regionId: number) {
  return queryFetcher<{ data: { region: RegionById } }>(MAIN_API_URL.concat(ENDPOINTS.region(regionId)));
}
